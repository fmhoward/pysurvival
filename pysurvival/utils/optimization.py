...
def optimize(loss_function, model, optimizer_str, lr=1e-4, nb_epochs=1000, 
    ...
    for epoch in range(nb_epochs):
        ...
        if verbose:
            bar.update( epoch + 1 )
            if epoch % 500 == 0:
                print(str(epoch) + ": " + str(loss_value))
...
def validateOptimize(loss_function, model, optimizer_str, VX_cens, VX_uncens, VY_cens, VY_uncens, VTriangle,
                     Vl2_reg, Vl2_smooth, lr=1e-4, nb_epochs=1000,
                    verbose=True, num_workers=0,  **kargs):
    """
    Providing the schema of the iterative method for optimizing a
    differentiable objective function for models that use gradient centric
    ...
    for epoch in range(nb_epochs):
        ...
        if verbose:
            bar.update(epoch + 1)
            loss2_value = loss_function(model, VX_cens, VX_uncens,VY_cens, VY_uncens, VTriangle,
                     Vl2_reg, Vl2_smooth).item()
            if epoch % 10 == 0:
                print(str(epoch) + ": " + str(loss_value) + " " + str(loss2_value))
    # Terminating the progressbar
    if verbose:
        bar.finish()
